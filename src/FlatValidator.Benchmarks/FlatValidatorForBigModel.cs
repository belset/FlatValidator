...
public class FlatValidatorForBigModel : FlatValidator<BigModel>
{
    public FlatValidatorForBigModel()
    {
        ...
        ErrorIf(m => m.Text1 is null || !m.Text1.Contains('a', StringComparison.InvariantCultureIgnoreCase), "Message T1", m => m.Text1);

        ErrorIf(m => m.Text2 is null, "Text2 cannot be null", m => m.Text2);
        ErrorIf(m => m.Text2 is null || !m.Text2.Contains('b', StringComparison.InvariantCultureIgnoreCase), "Message T2", m => m.Text2);

        ErrorIf(m => m.Text3 is null, "Text3 cannot be null", m => m.Text3);
        ErrorIf(m => m.Text3 is null || !m.Text3.Contains('c', StringComparison.InvariantCultureIgnoreCase), "Message T3", m => m.Text3);

        ErrorIf(m => m.Text4 is null, "Text4 cannot be null", m => m.Text4);
        ErrorIf(m => m.Text4 is null || !m.Text4.Contains('d', StringComparison.InvariantCultureIgnoreCase), "Message T4", m => m.Text4);

        ErrorIf(m => m.Text5 is null, "Text5 cannot be null", m => m.Text5);
        ErrorIf(m => m.Text5 is null || !m.Text5.Contains('e', StringComparison.InvariantCultureIgnoreCase), "Message T5", m => m.Text5);
        ...
        ErrorIf(m => m.StringCollection is null || !m.StringCollection.All(BigModel.IsValidStringInCollection), "String collection", m => m.StringCollection);

        ErrorIf(m => m.IntCollection is null, "Int collection cannot be null", m => m.IntCollection);
        ErrorIf(m => m.IntCollection is null || !m.IntCollection.All(BigModel.IsValidIntInCollection), "Int collection", m => m.IntCollection);

        ErrorIf(m => m.DecimalCollection is null, "Decimal collection cannot be null", m => m.DecimalCollection);
        ErrorIf(m => m.DecimalCollection is null || !m.DecimalCollection.All(BigModel.IsValidDecimalInCollection), "Decimal collection", m => m.DecimalCollection);
